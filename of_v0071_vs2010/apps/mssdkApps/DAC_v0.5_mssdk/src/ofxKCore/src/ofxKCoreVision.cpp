...
void ofxKCoreVision::loadXMLSettings()
{
	...
	//SECOND TUIO SERVER
	myTUIO1.bOSCMode			= XML.getValue("CONFIG:BOOLEAN:OSCMODE",1);
	myTUIO1.bTCPMode			= XML.getValue("CONFIG:BOOLEAN:TCPMODE",1);
	myTUIO1.bBinaryMode			= XML.getValue("CONFIG:BOOLEAN:BINMODE",1);
	myTUIO1.bHeightWidth		= XML.getValue("CONFIG:BOOLEAN:HEIGHTWIDTH",0);
	myTUIO1.setup(tmpLocalHost.c_str(), tmpPort+1, tmpFlashPort+1); //have to convert tmpLocalHost to a const char*
	//--------------------------------------------------------------
	//  END XML SETUP
}
...
void ofxKCoreVision::initDevice(){
	...
	camWidth		=	kinect.getDepthResolutionWidth();//.getWidth();
	camHeight		=	kinect.getDepthResolutionHeight();
}
...
void ofxKCoreVision::_update(ofEventArgs &e){
	...
	//	}
		...
		// Second Server
		myTUIO1.bOSCMode	= myTUIO.bOSCMode;
		myTUIO1.bTCPMode	= myTUIO.bTCPMode;
		myTUIO1.bBinaryMode = myTUIO.bBinaryMode;
		if (myTUIO1.bOSCMode || myTUIO1.bTCPMode || myTUIO1.bBinaryMode){
			myTUIO1.setMode(contourFinder.bTrackBlobs, contourFinder.bTrackFingers , contourFinder.bTrackObjects);
			myTUIO1.sendTUIO(tracker.getTrackedBlobsPtr(), tracker.getTrackedFingersPtr(), tracker.getTrackedObjectsPtr() );
		}
	}
}
...
void ofxKCoreVision::getPixels(){
    if(kinect.isFrameNew()) {
		sourceImg.setFromPixels(kinect.getDepthPixels());
        
        ofShortPixels depthRaw = kinect.getDistancePixels();
        ...
        for(int i = 0; i < numPixels; i++) {
            if((depthRaw[i] <= farThreshold) && (depthRaw[i] >= nearThreshold))
                depthPixels[i] = ofMap(depthRaw[i], nearThreshold, farThreshold, 255,0);
            else
                depthPixels[i] = 0;
        }
...
//	}
	...
	if(myTUIO.bOSCMode && myTUIO.bTCPMode)
		...
		sprintf(buf, "Host: %s\nProtocol: UDP [OSC]\nPort1: %i", myTUIO.localHost, myTUIO.TUIOPort);
	else if(myTUIO.bTCPMode){
		if(myTUIO.bIsConnected)
			sprintf(buf, "Host: %s\nProtocol: TCP [XML]\nPort1: %i", myTUIO.localHost, myTUIO.TUIOFlashPort);
		else
	...
	// SECOND SERVER
	if(myTUIO1.bOSCMode && myTUIO1.bTCPMode)
		sprintf(buf, "Dual Mode");
	else if(myTUIO1.bOSCMode)
		sprintf(buf, "\nPort2: %i", myTUIO1.TUIOPort);
	else if(myTUIO1.bTCPMode){
		if(myTUIO1.bIsConnected)
			sprintf(buf, "\nPort2: %i", myTUIO1.TUIOFlashPort);
		else
			sprintf(buf, "\nPort2: ----");
	}else if(myTUIO1.bBinaryMode){
		if(myTUIO1.bIsConnected)
			sprintf(buf, "\nPort2: %i", myTUIO1.TUIOFlashPort);
		else
			sprintf(buf, "\nPort2: ----");
	}
	verdana.drawString(buf, 573, 540);
