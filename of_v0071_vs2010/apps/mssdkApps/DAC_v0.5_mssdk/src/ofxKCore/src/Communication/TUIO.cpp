		...
		if(bBlobs){
			...
			if(blobBlobs->size() == 0){
				...
				for(blob = blobBlobs->begin(); blob != blobBlobs->end(); blob++) {
					...
					//if (blob->second.length > 500 && blob->second.length < 900)
					//	float perim = blob->second.length;
					
					/* uncomment only one of the following */
					//set.addFloatArg(blob->second.maccel);		// m
					//set.addFloatArg(blob->second.centroid.z);	// z
					set.addFloatArg(blob->second.length);		// perimeter
